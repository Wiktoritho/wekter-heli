...
export default {
  ...
  methods: {
    async fetchBlogs() {
      const response = await axios.get("/wekter-heli/src/jsons/blogs.json");
      ...
      if (this.highlightedDatas.length > 0) {
        ...
        this.highlightedPaths = latestHighlightedPosts.map((el) => `/wekter-heli/src/assets/images/blogs/${el.image}`);
      }
    ...
    applyFilters() {
      ...
      this.imagePaths = this.filteredData.map((el) => `/wekter-heli/src/assets/images/blogs/${el.image}`);
    },
